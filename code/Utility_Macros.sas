...
%MACRO create_CDI_flag(exposures=, outcomes=, output=);
	proc sql;
		CREATE TABLE &output AS
		SELECT exp.*, out.OUTCOME_DRUG_SVCDATE,
			CASE
				WHEN OUTCOME_DRUG_SVCDATE = . THEN 0
				ELSE 1
			END AS CDI_FLAG
		FROM &exposures AS exp LEFT JOIN &outcomes AS out
		ON exp.ENROLID = out.ENROLID AND exp.SVCDATE >= out.SVCDATE - 90;
	quit;
	
	*remove duplicates generated during merge step;
...
%MACRO add_hospitalization(exposures=, inpatients=, output=);
	proc sql;
		...
		ON exp.ENROLID = inp.ENROLID AND exp.SVCDATE <= inp.DISDATE + 90;
	quit;
...
*macro for joining and sorting to get unique patients;
%MACRO join_sort_unique_patients(data1=, data2=, output=);
	data &output;
		set &data1 &data2;
		keep ENROLID SEQNUM SVCDATE SEX YEAR AGEGRP REGION SEX DRUGNAME CDI_FLAG PRIOR_HOSPITALIZATION CCI_CAT;
	run;
...
%MACRO cont_enroll_prior_1(exposures=, enrollment=, output=);
	*prior year;
	data &output;
		merge &exposures(in=a keep=SEQNUM ENROLID SVCDATE YEAR REGION AGEGRP SEX DRUGNAME CDI_FLAG PRIOR_HOSPITALIZATION CCI_CAT) 
...
%MACRO cont_enroll_prior_0(exposures=, output=);
	*prior year;
	data &output;
		set &exposures(in=a keep=SEQNUM ENROLID SVCDATE YEAR REGION AGEGRP SEX DRUGNAME CDI_FLAG PRIOR_HOSPITALIZATION CCI_CAT);
		MONTH = month(SVCDATE);
		ENROLLED1 = 1;
...
%MACRO cont_enroll_curr(exposures=, enrollment=, output=);
	*current year;
	data &output;
		merge &exposures(in=a keep=SEQNUM ENROLID SVCDATE YEAR REGION AGEGRP SEX DRUGNAME CDI_FLAG PRIOR_HOSPITALIZATION CCI_CAT ENROLLED1 MONTH) 
			  &enrollment(in=b keep=ENROLID ENRIND1-ENRIND12);
		by ENROLID;
		if a;
		array ENRINDS (12) ENRIND1-ENRIND12;
...
%MACRO cont_enroll_after_1(exposures=, enrollment=, output=);
	*subsequent year;
	data &output;
		merge &exposures(in=a keep=SEQNUM ENROLID SVCDATE YEAR REGION AGEGRP SEX DRUGNAME CDI_FLAG PRIOR_HOSPITALIZATION CCI_CAT ENROLLED1-ENROLLED2 MONTH)
			  &enrollment(in=b keep=ENROLID ENRIND1-ENRIND12);
		by ENROLID;
		if a;
		MONTH = month(SVCDATE);
...
%MACRO cont_enroll_after_0(exposures=, output=);
	*subsequent year;
	data &output;
		set &exposures(keep=SEQNUM ENROLID SVCDATE YEAR REGION AGEGRP SEX DRUGNAME CDI_FLAG PRIOR_HOSPITALIZATION CCI_CAT ENROLLED1-ENROLLED2 MONTH);
...
%MACRO run_log_reg(data=, year=);
	data tmp.tmp_&year;	
		set &data;
		where YEAR = &year;
	run;
	
	proc logistic data=tmp.tmp_&year(keep = SEX AGEGRP REGION DRUGNAME CCI_CAT PRIOR_HOSPITALIZATION CDI_FLAG) plots=oddsratio(logbase=10) descending;
